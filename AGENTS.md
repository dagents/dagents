...
- **执行可取消 + 超时（2026-08-22，产品方案 B / 执行取消 spec）**：HTTP LLM 调用超时 `LLM_HTTP_TIMEOUT_MS`（默认 120s，流式为空闲看门狗）、inline CLI 静默看门狗 `INLINE_INACTIVITY_TIMEOUT_MS`（默认 300s）；显式取消 `POST /api/v1/chats/:id/cancel` 与 `POST /api/v1/workflows/runs/:runId/cancel` → `execution-registry.ts` 内存注册表（单进程红线）→ AbortSignal 贯穿引擎/llmClient/adapters（SIGTERM→SIGKILL）→ `chat:cancelled` WS 帧 + `persistCancelled`；gateway boot 清扫悬空 running（chats/runs→failed）。console 停止按钮接真取消。**dispatch/daemon 远程任务取消未做**（spec §7 Deferred）。适配器维护分级单源在 `packages/agent-adapters/src/tiers.ts`（core：claude/codex/qwen）。
