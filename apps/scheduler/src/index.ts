...
// propagates scheduler→gateway→workflow-engine without per-call-site header
// plumbing. The handle is awaited on shutdown to flush the BatchSpanProcessor so
// a SIGTERM doesn't drop the last in-flight batch.
 ...
 * `runs` table:
 *
 * - **Queue worker** — `startWorker` BRPOPs `dagents:tasks` and runs each
 *   task through the Workflow Engine API under the semaphore. Producers
 ...
 * - **HTTP fan-out** — `buildApp` exposes `POST /api/v1/scheduler/runs/fanout`,
 *   which creates a parent run + N child runs and gates each child's
 ...
 * ceiling).
 *
 * **Repro integration** — both paths also share one `ReproClient`
 ...
 * best-effort: a gateway/MinIO blip leaves a run unbound/unarchived (warned)
 * rather than failing it.
 *
 * **Reproduce** — `POST /api/v1/scheduler/runs/:runId/reproduce`
 ...
 * report. It reuses the artifact store for the report but is gated on
 ...
 *
 * **Restart recovery** — before the worker starts, `recoverStaleRuns`
...
// Prediction client for both paths: the worker posts one prediction per
// dequeued task; fan-out posts one per child. Both go through the gateway
// to the `@dagents/workflow` engine.
const prediction = createWorkflowPredictionClient({
  gatewayUrl,
  authorization: process.env.GATEWAY_AUTH,
})

// Repro integration: snapshot + bind + archive on every run. The store
...
// Restart recovery: clear any leaked semaphore slots and re-enqueue
...
// Artifact store for the reproduce route: archives the JSON comparison
// report as the re-run row's `artifact_uri`. Reuses the same store the
