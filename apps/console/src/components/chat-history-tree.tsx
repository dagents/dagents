...
/**
 * ChatHistoryTree —— 以项目目录为第一维度的会话历史树（从 ChatNavSidebar
 * 抽出的共享组件，2026-08-29）。
 *
 * deepseek session-browser 范式：34px 目录头（folder ↔ chevron hover 互换）
 * + 32px 会话行（状态点 + 标题 + 紧凑相对时间，hover 让位给重命名/删除
 * 操作）、每组 5 行 + 「显示更多」溢出控制、活动会话所在目录导航时自动
 * 展开一次。目录头带每目录新建 ➕ 与目录操作菜单（重命名/删除）。
 *
 * 两个侧栏共用（单一来源）：
 *   - ChatNavSidebar（Chat-First 回滚壳）：主导航下方
 *   - AppNavSidebar（Workflow-First 主壳）：主导航下方 —— 用户裁决恢复
 *     「项目为第一维度」的会话树，取代扁平「最近对话」列表
 *
 * 样式全部复用 chat-nav-sidebar.css 的 .chat-nav-browser 与
 * .chat-nav-dir- / .chat-nav-chat- 系列类（含 .chat-layout-sidebar.collapsed
 * 折叠态规则）。
 */
...
export function ChatHistoryTree(): React.ReactElement {
  ...
  // tree reflects the new state without a full page refresh.
  ...
  // Inline add-directory flow — lives in the tree header so the user can add
  // a directory from anywhere without going to /directories.
