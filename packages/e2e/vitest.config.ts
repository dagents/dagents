...
// The M6.7 full-chain trace e2e. One test file: it boots the real dispatch +
...
//
// DB + Redis: the docker-compose dev stack (127.0.0.1:15432 / :16479). One
// file only, so `fileParallelism: false` is belt-and-suspenders against any
// future sibling file racing the shared `runs` / `dispatch_tasks` tables.
// `setupFiles` runs `setup.ts` once per file: env defaults + pending migrations
// so `runs` / `run_node_spans` exist on a fresh DB (same pattern as
// apps/scheduler + packages/repro).
